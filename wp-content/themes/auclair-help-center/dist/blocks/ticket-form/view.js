 ...
 */
function validateField(context, field) {
  if ('category' === field) {
  ...
  if ('subject' === field) {
    ...
    return context.subject.length > context.subjectMax ? `Subject must be ${context.subjectMax} characters or fewer.` : undefined;
  }
  if ('description' === field) {
    ...
    return value.length < context.descriptionMin ? `Please add at least ${context.descriptionMin} characters so we can help.` : undefined;
  }
...
const FIELDS = ['category', 'subject', 'description', 'email'];
 ...
 */
function refreshField(context, field, live) {
  const message = validateField(context, field);
  const {
    [field]: current,
    ...rest
...
(0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_1__.store)('auclair', {
  state: {
  ...
  actions: {
    ...
    closeCategoryOnOutsideClick(event) {
      ...
      const {
        ref
      } = (0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_1__.getElement)();
    ...
    closeCategoryOnEscape(event) {
      ...
      if (context.categoryOpen) {
        context.categoryOpen = false;
        (0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_1__.getElement)().ref.focus();
      }
    },
    selectCategory() {
      const context = (0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_1__.getContext)();
    ...
    setFile(event) {
      ...
      if (context.allowedTypes.length && !context.allowedTypes.includes(file.type)) {
        context.errors = {
          ...context.errors,
          attachment: 'That file type is not supported.'
        };
        input.value = '';
        selectedFile = null;
        context.fileName = '';
      ...
      if (file.size > context.maxUploadBytes) {
        context.errors = {
          ...context.errors,
          attachment: 'That file is too large.'
        };
        input.value = '';
        selectedFile = null;
        context.fileName = '';
    ...
    *submitTicket(event) {
      ...
      if (Object.keys(errors).length > 0) {
        ...
        context.touched = FIELDS.reduce((acc, field) => ({
          ...acc,
          [field]: true
        }), {
          ...context.touched
