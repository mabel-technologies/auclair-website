...
```bash
npm run scaffold:block -w=au-clair-help-center-theme
```
...
```json
{
  ...
  "textdomain": "au-clair-help-center-theme",
  "name": "tenup/my-block",
  "icon": "feedback",
  "category": "formatting",
  "attributes": {
 ...
 * Block markup
 *
 * @package AuclairHelpCenter\Blocks\MyBlock
 *
...
/**
 * Title: My Pattern
 * Slug: au-clair-help-center-theme/my-pattern
  ...
  return (
    <>
      <BlockControls group="block">
        <ToolbarButton
          icon={alignLeft}
          title={__('Align Left', 'au-clair-help-center-theme')}
          onClick={() => setAttributes({ alignment: 'left' })}
        />
      </BlockControls>
      <InspectorControls group="settings">
        <PanelBody title={__('Block Settings', 'au-clair-help-center-theme')}>
      ...
      <div {...blockProps}>
        <RichText
          ...
          value={title}
          onChange={(title) => setAttributes({ title })}
          placeholder={__('Enter title...', 'au-clair-help-center-theme')}
        />
      </div>
    </>
  );
