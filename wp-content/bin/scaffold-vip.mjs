...
function validateSource() {
	const required = [
		join(SOURCE, 'mu-plugins', 'auclair-core'),
		join(SOURCE, 'themes', 'auclair-help-center'),
		join(SOURCE, 'themes', 'auclair-help-center'),
...
function writePackageJson(targetPath) {
	...
	ourPkg.workspaces = ourPkg.workspaces.map((ws) =>
		ws.replace('mu-plugins/auclair-core', 'client-mu-plugins/auclair-core'),
	);
...
function writePhpstanNeon(targetPath, isBlock) {
	const themePath = isBlock ? 'themes/auclair-help-center' : 'themes/auclair-help-center';
...
parameters:
    paths:
        - ${themePath}
        - client-mu-plugins/auclair-core
`;
...
function updatePluginLoader(targetPath) {
	...
	if (content.includes('auclair-core/plugin.php')) {
	...
	if (!content.endsWith('\n')) content += '\n';
	content += `\nrequire __DIR__ . '/auclair-core/plugin.php';\n`;
...
async function main() {
	...
	// -- Copy chosen theme --
	const themeSrcName = isBlock ? 'auclair-help-center' : 'auclair-help-center';
	...
	const pluginSrc = join(SOURCE, 'mu-plugins', 'auclair-core');
	const pluginDest = join(TARGET, 'client-mu-plugins', 'auclair-core');
	if (existsSync(pluginSrc)) {
		mkdirSync(join(TARGET, 'client-mu-plugins'), { recursive: true });
		cpSync(pluginSrc, pluginDest, { recursive: true });
		console.log('  Copied mu-plugins/auclair-core/ -> client-mu-plugins/auclair-core/');
	}
	...
	if (existsSync(phpstanConstantsPath)) {
		...
		if (isBlock) {
			constContent = constContent
				.split('\n')
				.filter((line) => {
					if (line.includes('AU_CLAIR_HELP_CENTER_THEME_')) return true;
					if (line.includes('AU_CLAIR_HELP_CENTER_THEME_')) return false;
					return true;
				})
				.join('\n');
		} else {
			constContent = constContent
				.split('\n')
				.filter((line) => {
					return !line.includes('AU_CLAIR_HELP_CENTER_THEME_');
				})
				.join('\n');
		}
	...
	const oldPluginDir = join(TARGET, 'client-mu-plugins', 'auclair-core');
	...
	// Rename .pot files
	const pluginPotOld = join(newPluginDir, 'languages', 'AuclairCore.pot');
	...
	if (existsSync(pluginPotOld) && pluginPotOld !== pluginPotNew) {
		...
		console.log(`  Renamed AuclairCore.pot to ${values.pluginNamespace}.pot`);
	}

	if (!isBlock) {
		const themePotOld = join(newThemeDir, 'languages', 'AuclairHelpCenter.pot');
		...
		if (existsSync(themePotOld) && themePotOld !== themePotNew) {
			...
			console.log(`  Renamed AuclairHelpCenter.pot to ${values.themeNamespace}.pot`);
		}
	}
	...
	if (existsSync(renamedPluginPkg)) {
		...
		if (isBlock) {
			pluginPkgContent = pluginPkgContent.replaceAll('auclair-help-center', values.themeSlug);
			pluginPkgContent = pluginPkgContent.replaceAll('auclair-help-center', values.themeSlug);
		} else {
			pluginPkgContent = pluginPkgContent.replaceAll('auclair-help-center', values.themeSlug);
		}
