 ...
 *
 * Downloads the reference auclair-core from GitHub and scaffolds it into an
...
async function downloadPluginFromGitHub(ref) {
	console.log(`\n  Downloading auclair-core from GitHub (ref: ${ref})...\n`);

	const tarballUrl = `https://api.github.com/repos/au-clair-help-center/au-clair-help-center/tarball/${ref}`;
	...
	await pipeline(
		...
		extract({
			...
			filter: (path) => {
				// Only extract mu-plugins/auclair-core/ and mu-plugins/auclair-core-loader.php
				return path.includes('/mu-plugins/auclair-core/') || path.endsWith('/mu-plugins/auclair-core-loader.php');
			},
		}),
	);
...
async function main() {
	...
	replacements.push(['au-clair-help-center/auclair-core', `${values.composerVendor}/${values.pluginSlug}`]);
	...
	if (values.description) {
		replacements.push(['AuClair Help Center — WordPress knowledge base and support site', values.description]);
	}
	...
	const tempPluginDir = join(TEMP_DIR, 'auclair-core');
	const tempLoaderFile = join(TEMP_DIR, 'auclair-core-loader.php');
	...
	if (existsSync(tempLoaderFile)) {
		...
		loaderContent = loaderContent.replaceAll('auclair-core', values.pluginSlug);
